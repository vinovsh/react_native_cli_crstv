...
const AddVideoScreen = props => {
  ...
  const [videoUrl, setVideoUrl] = React.useState(
    props.route.params.item.assets[0].uri,
  );
  ...
  const [compressedVideoUrl, setCompressedVideoUrl] = React.useState(null);
    ...
    if (video && title && image) {
      try {
        ...
        if (result !== null) {
          ...
          let Video_filtred = result.replace('file://', '');
          let reday_upload_video = 'file:///' + Video_filtred;

          setCompressedVideoUrl(reday_upload_video);
          // setVideoUrl(result);
          // upload(result);
          console.log(reday_upload_video);
        }
      } catch (e) {
      ...
      Alert.alert('Wrong Input!', 'Plz fill all details', [{text: 'Okay'}]);
    }
  };
  ...
  const upload = async recived_video => {
    ...
    if (video && title && image) {
      ...
      try {
        await axios
          .post(config.BASE_URL + 'reels_upload', formData, {
    ...
    <View style={styles.container}>
      <Modal visible={progressModal}>
        <View
          ...
          <Text style={{fontSize: 18, color: '#000', margin: 10}}>
            Compressing...
          </Text>
        </View>
      </Modal>
        ...
        {uploading ? (
          <TouchableOpacity>
            <Text style={{textAlign: 'center', fontSize: 20, color: '#fff'}}>
              <ActivityIndicator size="small" color="#fff" /> Uploading..
            </Text>
          </TouchableOpacity>
        ) : (
          <TouchableOpacity
